...
KLINE_SECIDS = {
    ...
    'rt_hkHSI':    '100.HSI',
    'gb_dji':      '100.DJIA',
    'gb_ixic':     '100.IXIC',  # 纳斯达克综合指数(IXIC) 非NDX(纳斯达克100)
    'gb_inx':      '100.SPX',
    'gb_n225':      '100.N225',
    'b_KOSPI':     '100.KS11',
    'b_DAX':       '100.GDAXI',
...
# 百度专用 SSL context（跳过证书验证，避免某些环境下 403/SSL 错误）
_baidu_ssl_ctx = ssl.create_default_context()
_baidu_ssl_ctx.check_hostname = False
_baidu_ssl_ctx.verify_mode = ssl.CERT_NONE


def _http_get_baidu(url, decode='utf-8', timeout=10, retries=2):
    """百度专用 HTTP GET，带百度 Referer / headers / SSL context"""
    last_err = None
    for attempt in range(retries + 1):
        try:
            req = urllib.request.Request(url, headers={
            ...
            resp = urllib.request.urlopen(req, timeout=timeout,
                                          context=_baidu_ssl_ctx)
            return resp.read().decode(decode, errors='replace')
        except Exception as e:
            last_err = e
            if attempt < retries:
                time.sleep(0.5 * (attempt + 1))
    _log(f'HTTP GET (Baidu) failed ({retries+1} tries): {url[:80]} — {last_err}')
    return None
...
def fetch_all_data():
    ...
    + 新浪gi分时 + 百度财经
    所有数据源并行获取，交叉验证，少数服从多数"""

    with ThreadPoolExecutor(max_workers=6) as pool:
        f_sina    = pool.submit(_fetch_sina_realtime)
        f_em      = pool.submit(_fetch_em_realtime, [item[0] for item in INSTRUMENTS])
        f_tencent = pool.submit(_fetch_tencent_quote, TENCENT_QUOTE)
        f_fx678   = pool.submit(_fetch_fx678_quote)
        f_gi      = pool.submit(_fetch_sina_gi_realtime)
        f_baidu   = pool.submit(_fetch_baidu_quote)

        sina_data    = f_sina.result()
        em_data      = f_em.result()
        tencent_data = f_tencent.result()
        fx678_data   = f_fx678.result()
        gi_data      = f_gi.result()
        baidu_data   = f_baidu.result()
...
def _fetch_futures_minute_kline(code, klt, count):
    ...
    try:
        ...
        # akshare 可能在网络层无限阻塞，用子线程 + 超时保护
        _ak_q = _queue.Queue()
        def _ak_worker():
            try:
                _ak_q.put(ak.futures_zh_minute_sina(symbol=ak_symbol))
            except Exception as ex:
                _ak_q.put(ex)
        t_ak = threading.Thread(target=_ak_worker, daemon=True)
        t_ak.start()
        try:
            raw_result = _ak_q.get(timeout=20)  # 最多等20秒
        except _queue.Empty:
            _log(f'akshare timeout (20s) for {ak_symbol}')
            return None
        if isinstance(raw_result, Exception):
            _log(f'akshare error for {ak_symbol}: {raw_result}')
            return None
        df = raw_result
...
def fetch_kline_data(code, klt='101', count=90):
    ...
    数据源优先级：
    - 商品(黄金/白银/原油): 东方财富 → 新浪期货/akshare → 新浪
    - 指数(9个): 百度K线(第一源，稳定且真实指数) → 东方财富 → 新浪/腾讯
    """
    secid = KLINE_SECIDS.get(code)

    # 除商品外，百度优先（百度稳定、纳斯达克为真实IXIC综合指数）
    if code in BAIDU_KLINE_MAP:
        data = _fetch_baidu_kline(code, klt, count)
        if data:
            return data, '百度K线'

    # 商品无百度源，东方财富降为第一备选（注意：EM易宕机，仅作快速尝试）
    if secid:
    ...
    if klt not in ('5', '15', '30', '60'):
        return None, None
    # 分时K线多源fallback：
    # 1. 百度K线（全9指数优先：us/hk/cn原生分时K线，jp/kr/de用1分钟序列聚合）
    # 2. 新浪A股（上证/深证备用）
    # 3. 新浪期货→akshare（黄金/白银/原油）
    # 4. 新浪全球指数分时聚合（日经/韩国/德国备用）
    # 5. 百度getbanner分时聚合（恒生备用）
    if code in BAIDU_KLINE_MAP:
        data = _fetch_baidu_kline(code, klt, count)
        if data:
            return data, '百度K线'
...
# 百度getquotation覆盖的品种：全部9个指数（美股三大指数/恒生/上证/深证/日经/韩国/德国）
# (百度code, 名称, market_type)
# 已验证:
#   us: DJI/IXIC/SPX    全周期(min5~month)
#   hk: HSI             全周期
#   cn: 000001/399001   全周期
#   jp: NK225           day/week/month（无分时）
#   kr: KOSPI           day/week/month（无分时）
#   de: DAX             day/week/month（无分时）
# 注意: 纳斯达克用 IXIC(综合指数)，与实时行情一致（不能用 NDX/NQ 期货）
# 百度不覆盖: 商品期货（黄金/白银/原油）
BAIDU_KLINE_MAP = {
    'gb_dji':      ('DJI',    '道琼斯',   'us'),
    'gb_ixic':     ('IXIC',   '纳斯达克', 'us'),
    'gb_inx':      ('SPX',    '标普500',  'us'),
    'rt_hkHSI':    ('HSI',    '恒生指数', 'hk'),
    's_sh000001':  ('000001', '上证指数', 'cn'),
    's_sz399001':  ('399001', '深证成指', 'cn'),
    'gb_n225':     ('NK225',  '日经225',  'jp'),
    'b_KOSPI':     ('KOSPI',  '韩国综指', 'kr'),
    'b_DAX':       ('DAX',    '德国DAX',  'de'),
}
...
def _fetch_baidu_kline(code, klt, count):
    ...
    覆盖 全部9个指数品种（美股三大指数/恒生/上证/深证/日经/韩国/德国）
    支持 5/15/30/60分 + 日/周/月K
    - us/hk/cn: quotation_index_kline 直接提供各周期K线
    - jp/kr/de: 日/周/月K用 kline；分时K线无原生数据，
                用 quotation_index_minute 的1分钟价格序列聚合成 OHLC
    """
    if code not in BAIDU_KLINE_MAP:
        return None
    baidu_code, baidu_name, market_type = BAIDU_KLINE_MAP[code]
    ...
    # 1) 先试原生K线（quotation_index_kline）：us/hk/cn 全周期，jp/kr/de 日/周/月
    result_list = _baidu_kline_group(baidu_code, baidu_name, market_type, ktype)
    # 2) 若失败且是分时K线，用分时走势（quotation_index_minute）聚合
    #    （jp/kr/de 无原生分时K线，但提供1分钟分时序列）
    if not result_list and klt in ('5', '15', '30', '60'):
        minute_list = _baidu_minute_group(baidu_code, baidu_name, market_type)
        if minute_list:
            result_list = _aggregate_minute_to_ohlc(minute_list, int(klt))
...
def _baidu_kline_group(baidu_code, baidu_name, market_type, ktype):
    """百度 quotation_index_kline：解析OHLC K线（str/list 格式）"""
    url = (f'https://finance.pae.baidu.com/vapi/v1/getquotation?'
           f'srcid=5353&pointType=string&group=quotation_index_kline'
           f'&query={baidu_code}&code={baidu_code}&market_type={market_type}'
           f'&newFormat=1&name={urllib.parse.quote(baidu_name)}&is_kc=0'
           f'&ktype={ktype}')
    body = _http_get_baidu(url, timeout=10, retries=2)
    if not body:
        return None
    try:
        j = json_module.loads(body)
    ...
    # marketData 是分号分隔字符串或列表，每条记录用逗号分隔CSV:
    # "ts,time,open,close,vol,high,low,...;..."
...
def _baidu_minute_group(baidu_code, baidu_name, market_type):
    """获取百度分时走势（quotation_index_minute）1分钟价格序列
    返回 [(datetime, price), ...]
    """
    url = (f'https://finance.pae.baidu.com/vapi/v1/getquotation?'
           f'srcid=5353&pointType=string&group=quotation_index_minute'
           f'&query={baidu_code}&code={baidu_code}&market_type={market_type}'
           f'&newFormat=1&name={urllib.parse.quote(baidu_name)}&is_kc=0')
    body = _http_get_baidu(url, timeout=10, retries=2)
    if not body:
    ...
    # 分时格式: marketData 可能是 dict {"date":..., "p":"ts,time,price,...;..."}
    # 或 list[dict]（jp/kr/de 返回 [{date, p}]）
    # 或字符串列表。每条记录: ts,time,price,涨跌额,涨跌幅,vol,amount,totalVol,totalAmount
    # fields: [0]=timestamp, [1]=time, [2]=price
    if isinstance(raw, dict):
        ...
        # jp/kr/de: marketData = [{"date":"...", "p":"ts,time,price;..."}]
        p_str = raw[0].get('p', '')
        if isinstance(p_str, list):
            p_str = ''.join(p_str)
        records = p_str.split(';')
    ...
    for entry in records:
        ...
        try:
            ...
            if price <= 0:
                continue
            dt = _dt3.fromtimestamp(ts)
            result_list.append((dt, price))
...
def _aggregate_minute_to_ohlc(timed_prices, period):
    """将1分钟价格序列聚合成 period 分钟 OHLC K线
    timed_prices: [(datetime, price), ...]
    """
    ...
    while i < n:
        ...
        aligned_min = (dt_start.minute // period) * period
        bucket_start = dt_start.replace(minute=aligned_min, second=0, microsecond=0)
        bucket_end = bucket_start + _td4(minutes=period)
        bucket_prices = []
        j = i
...
class KLineWindow:
    ...
    KEY_PERIODS = ['5', '15', '30', '60', '101', '102', '103']  # 键1-7对应周期
    ...
    def __init__(self, parent_root, code, name, on_close_cb=None, latest_quote=None):
        ...
        self._source_name = ''
        self._cross_items = []       # 十字光标 item 列表
        self._hover_box = None       # OHLC 浮动框背景 item
        self._hover_texts = []       # OHLC 浮动框文字 items
        self._draw_params = None     # _draw() 保存的绘图参数
        self._title_timer = None     # 标题价格定时器
    ...
    def _build_ui(self):
        ...
        # 十字光标：鼠标在K线区域移动时显示
        cv.bind('<Enter>', self._on_kline_enter)
        cv.bind('<Leave>', self._on_kline_leave)
        # 键盘快捷键：1-7切换周期
        self.win.bind('<Escape>', lambda e: self._on_close())
        for idx in range(7):
            self.win.bind(str(idx + 1), lambda e, i=idx: self._switch_period(self.KEY_PERIODS[i]))
    ...
    def _draw(self):
        ...
        # 保存绘图参数供十字光标用
        self._draw_params = {
            'x0': x0, 'x1': x1, 'k_y0': k_y0, 'k_y1': k_y1, 'v_y0': v_y0, 'v_y1': v_y1,
            'plot_w': plot_w, 'bar_step': bar_step, 'n': n,
            'p_max': p_max, 'p_min': p_min, 'p_range': p_range,
            'p2y': p2y, 'v2y': v2y,
            'is_minute': is_minute,
        }
        # 重新填充十字光标（如果有数据且鼠标在K线区）
        self._clear_crosshair()
    ...
    def _clear_crosshair(self):
        ...
        if self._hover_box is not None:
            try: self.cv.delete(self._hover_box)
            except tk.TclError: pass
            self._hover_box = None
        for t in self._hover_texts:
            try: self.cv.delete(t)
            except tk.TclError: pass
        self._hover_texts = []

    def _on_kline_enter(self, event):
        pass  # 鼠标进入画布时不做特殊处理
    ...
    def _update_crosshair(self, mx, my):
        """更新十字光标和OHLC浮动提示"""
        self._clear_crosshair()
        if not self.data or not self._draw_params:
            return
        dp = self._draw_params
        x0, x1 = dp['x0'], dp['x1']
        k_y0, k_y1 = dp['k_y0'], dp['k_y1']
        v_y0, v_y1 = dp['v_y0'], dp['v_y1']
        plot_w, bar_step, n = dp['plot_w'], dp['bar_step'], dp['n']
        p2y, v2y = dp['p2y'], dp['v2y']
        # 只在K线图+成交量区域显示
        if my < k_y0 - 5 or my > v_y1 + 5:
            return
        if mx < x0 - 5 or mx > x1 + 5:
            return
        # 找到最近的K线索引
        idx = int((mx - x0) / bar_step - 0.5)
        idx = max(0, min(n - 1, idx))
        cx = x0 + bar_step * (idx + 0.5)
        d = self.data[idx]
        # ── 竖线（从K线区顶到底部）──
        self._cross_items.append(self.cv.create_line(cx, k_y0, cx, v_y1,
            fill=C_DIM, width=1, dash=(2, 3)))
        # ── 横线（跟随鼠标Y，仅在K线图区域）──
        h_y = max(k_y0, min(my, k_y1))
        self._cross_items.append(self.cv.create_line(x0, h_y, x1, h_y,
            fill=C_DIM, width=1, dash=(2, 3)))
        # 横线对应的价格（右上角显示）
        if dp['p_range'] > 0:
            h_price = dp['p_max'] - (h_y - k_y0) / (k_y1 - k_y0) * dp['p_range']
            self._cross_items.append(self.cv.create_text(self.WIN_W - 4, h_y,
                text=f'{h_price:.2f}', fill=C_ACCENT,
                font=('Consolas', 10), anchor='ne'))
        # ── OHLC 浮动提示框 ──
        o, c, h, l = d['open'], d['close'], d['high'], d['low']
        v = d.get('volume', 0)
        date_str = d['date']
        chg_val = ((c - o) / o * 100) if o else 0
        chg_color = C_RED if c >= o else C_GREEN
        lines = [
            date_str,
            f'开 {o:.2f}',
            f'高 {h:.2f}',
        ...
        if v > 0:
            lines.append(f'量 {v:,.0f}')
        # 浮动框位置：默认在鼠标右侧，超出边界则放左侧
        box_w = 120
        box_h = len(lines) * 16 + 10
        box_x = cx + 12
        if box_x + box_w > x1 - 2:
            box_x = cx - 12 - box_w
        box_y = max(k_y0 + 2, min(h_y - box_h // 2, k_y1 - box_h - 2))
        self._hover_box = self.cv.create_rectangle(box_x, box_y, box_x + box_w, box_y + box_h,
            fill='#1e293b', outline=C_BORDER, width=1, stipple='')
        for i, line in enumerate(lines):
            color = '#ffffff' if i == 0 else chg_color if '幅' in line else '#cbd5e1'
            if i == 0:
                color = C_GOLD
            tid = self.cv.create_text(box_x + 8, box_y + 8 + i * 16,
                text=line, fill=color, font=('Consolas', 10), anchor='nw')
            self._hover_texts.append(tid)
    ...
    def _start_title_refresh(self):
        ...
        def worker():
            ...
            try:
                # 使用新浪实时行情单品种获取
                alias = SINA_REALTIME_ALIAS.get(self.code, self.code)
                url = f'http://hq.sinajs.cn/list={alias}'
                body = _http_get(url, decode='gbk', timeout=8, retries=1)
                if body:
                    for line in body.strip().split('\n'):
                        line = line.strip()
                        ...
                        if not data_str:
                            continue
                        # 根据 code 找到品种格式（简化：直接用 INSTRUMENTS）
                        name, fmt = None, None
                        for c, n, f in INSTRUMENTS:
                            if c == self.code:
                                name, fmt = n, f
                                break
                        if not fmt:
                        ...
                        parsed = _parse(name, fmt, data_str)
                        if parsed and parsed.get('price', 0) > 0:
                            self._latest_quote = parsed
                            if self.running:
                                self.win.after(0, self._update_title_quote)
                        break
            except Exception:
                pass
            # 15秒后再拉一次
            if self.running:
                self._title_timer = threading.Timer(15.0, worker)
                self._title_timer.daemon = True
    ...
    def _on_close(self):
        ...
        if self._title_timer:
            try: self._title_timer.cancel()
            except Exception: pass
...
class ValuationWindow:
    ...
    def _build_ui(self):
        ...
        # 拖拽提示
        cv.create_text(self.WIN_W - 55, 22, text='标题栏拖拽', fill='#64748b',
...
class FinanceWidget:
    ...
    def __init__(self, root):
        ...
        self._mini_idx = 0  # mini模式当前显示品种索引
        self._fetch_lock = threading.Lock()  # 并发刷新去重锁
        pos = self._load_pos()
    ...
    def _on_wheel(self, event):
        ...
        if not self.is_mini:
            return
        if not self._sorted_codes:
            return
        n = len(self._sorted_codes)
        if event.delta > 0:
            self._mini_idx = (self._mini_idx - 1) % n
        else:
            self._mini_idx = (self._mini_idx + 1) % n
        # 更新mini显示到指定品种
        self._update_mini_to_idx(self._mini_idx)

    def _update_mini_to_idx(self, idx):
        """强制更新mini窗口显示sorted_codes中指定索引的品种"""
        if not self._sorted_codes or idx >= len(self._sorted_codes):
            return
        top_code = self._sorted_codes[idx]
        self._mini_top_code = top_code
        top_item = next((it for it in self._latest_items if it['code'] == top_code), None)
        if top_item:
            icon = MINI_ICONS.get(top_code, '\u25C8')
            ...
            while self.running:
                if self._fetch_lock.acquire(blocking=False):
                    try:
                        ...
                        self._fetch_lock.release()
                # 如果锁被占用（_force_refresh正在执行），跳过本轮
                if self._fail_count >= 2:
                    wait_ms = 5000
                else:
